        ...
        <div className="min-h-screen bg-[#110F17] text-white px-4 md:px-10 lg:px-44">
            <div className="py-16">
                    ...
                    >
                        <option value="">Select Year</option>
                        {
                            [2024, 2023, 2022, 2021, 2020, 2019].map((year: number) => (
                                <option value={year} key={year} className="bg-[#171D22]">{year}</option>
                            ))
                        }
                    </select>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between">
                    <h1 className="text-lg md:text-6xl font-bold">{year} {genre === "All" ? "" : genre} Movie List</h1>
                    <div className="flex flex-wrap space-x-0 md:space-x-4 mt-4 md:mt-0">
                        {
                            ['All', 'Animation', 'Comedy', 'Fantasy'].map((genreName: string, key: number) => (
                                <Button title={genreName} key={key} action={() => setGenre(genreName)} />
                            ))
                        }
                    </div>
                </div>
